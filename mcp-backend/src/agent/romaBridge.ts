...
export async function planTask(goal: string, context?: Record<string, unknown>, strategy?: 'react' | 'cot' | 'code_act'): Promise<PlanResponse> {
  const client = getRomaClient();
  return client.plan({ goal, context, strategy });
}

export async function actOnTask(task: string, context?: Record<string, unknown>, tools?: Array<Record<string, unknown>>): Promise<ActResponse> {
  const client = getRomaClient();
